...
- Dedicated input contracts for create and update operations
...
- A complete in-memory CRUD flow (Create, Read, Update, Delete)
...
| PUT  | `/products/{id}`  | Updates an existing product by its ID. The request body replaces the current product data while preserving the original ID. Returns the updated product or `404 Not Found` if the product does not exist. |
| DELETE | `/products/{id}` | Deletes a product by its ID. Returns `204 No Content` when successful, or `404 Not Found` if the product does not exist. |

These routes establish a **complete CRUD HTTP flow**, fully implemented using Go’s standard library and a clean, layered architecture.
